...
//plainText 明文
//key 秘钥，大小为8byte
func DESCBCEncrypt(plainText []byte, key []byte) (cipherText []byte, err error) {
...
func DESCBCDecrypt(cipherText, key []byte) (plainText []byte, err error) {
...
func main() {
	...
	//明文
	plainText := []byte("你好呀")
	//秘钥(长度必须为8字节，因为DES加密就是按照8字节(64bit)一组加密的，规定秘钥长度和每组要加密的长度相等)
	key := []byte("12345678")
	cipherText, err := DESCBCEncrypt(plainText, key)
	if err != nil {
		log.Fatalf("加密出错,错误为:%v\n", err)
	}
	fmt.Println(string(cipherText))
	cipherDecryptText, err := DESCBCDecrypt(cipherText, key)
	if err != nil {
