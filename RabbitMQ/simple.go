...
//创建简单模式的RabbitMQ实例
func NewSimpleRabbitMQ(url string, queueName string) *rabbitMQ {
	return newRabbitMQ(url, queueName, "", "", "simple")
}

//简单模式：生产者客户端
func (this *rabbitMQ) simpleSend(message string) {
	...
	this.failOnError(err, "发送消息失败")
}

//简单模式：消费者客户端
func (this *rabbitMQ) simpleConsume(handleFunc HandleFunc) {
	queue, err := this.channel.QueueDeclare(
		this.queueName,  //队列名称
		false,   //消息持久化
		false, //是否自动删除
